...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query("SELECT MIN(p.price) FROM Product p INNER JOIN p.vendor v " +
            "WHERE p.category.id = :categoryId AND v.province = :province AND v.city = :city")
    Long findMinPrice(String categoryId, String province, String city);
    @Query("SELECT MAX(p.price) FROM Product p INNER JOIN p.vendor v " +
            "WHERE p.category.id = :categoryId AND v.province = :province AND v.city = :city")
